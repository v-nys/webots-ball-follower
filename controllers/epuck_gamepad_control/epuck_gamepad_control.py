...
if len(sys.argv) != 2:
    print("Er moet exact één argument zijn, namelijk het devicenummer van de gamepad. Check dat via `evtest`.")
    sys.exit(1)
...
while robot.step(TIME_STEP) != -1:
    ...
    horizontal = max(min(abs_info_axis_0 / (32768.0 if abs_info_axis_0 < 0 else 32767.0),1), -1)
    vertical = -max(min(abs_info_axis_1 / (32768.0 if abs_info_axis_1 < 0 else 32767.0), 1), -1)
